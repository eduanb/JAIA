...
public class RandomResetClampedGbestPSO implements PSO,PSOConstants
{

	Random generator;

	public RandomResetClampedGbestPSO()
	{
			...
			for (int i = 0; i < SWARM_SIZE; i++)
			{
				...
				for (int j = 0; j < problemSet.getProblemDimention(); j++)
				{
					...
					if(newVel[j] > problemSet.getClampMaxValues()[j])
						newVel[j] = (-problemSet.getClampMaxValues()[j]) + generator.nextDouble() * (problemSet.getClampMaxValues()[j]);
				}
