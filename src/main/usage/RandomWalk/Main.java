...
public class Main
{
    ...
    static void doFDC(PatternFile patternFile, NeuralNetwork[] NNs)
    {
        ...
        double errorAvgDist = HelperFunctions.average(ErrorDist);
        double MSEAvgDist = HelperFunctions.average(MSEDist);
        double errorAvgFit = HelperFunctions.average(fitnessError);
        double MSEAvgFit = HelperFunctions.average(fitnessMSE);
        double totalError = 0;
        double totalMSE = 0;
        for (int i = 0; i < FDC_COUNT; i++) {
